...
describe('SearchParams Unit Tests', () => {
    ...
    test.each([
        ...
        expect(params.sort_dir).toBeNull();
        expect(new SearchParams({ sort: 'field', sort_dir: props.sort_dir as any }).sort_dir).toBe(props.expected);
    });
    ...
    test.each([
        ...
        expect(new SearchParams({ filter: props.filter as any }).filter).toBe(props.expected);
    });
});
